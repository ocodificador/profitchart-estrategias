...
begin
  ...
  if Abs(QIND) <= AOTW_Limit then Color := clGray
  else if (Periods > 0) then Color := clGreen
  else Color := clRed;

  if (Color = clRed) and (Lowest(QIND,2) = QIND) then Color := clWhite;
  if (Color = clGreen) and (Highest(QIND,2) > QIND) then Color := clWhite; 

  if Periods < 0 then QIND := Neg(QIND);  
  Plot(QIND);
